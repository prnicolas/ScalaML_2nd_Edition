 ...
 * Version 0.99.2
 */
package org.scalaml.supervised.nnet.mlp
 ...
 * @version 0.99.2
 * @see Scala for Machine Learning Chapter 10 Multi-layer perceptron
 */
@throws(classOf[IllegalArgumentException])
private[scalaml] class MLPModel(val synapses: Vector[MLPConnSynapses]) extends Model[MLPModel] {
  require(synapses.nonEmpty, "MLPModel synapses undefined")

  final def getSynapses(n: Int): MLPConnSynapses = synapses(n)

  final def getTopology: Array[Int] = Array[Int](synapses.head.head.length) ++ synapses.map(_.head.length)
...
private[scalaml] object MLPModel {
