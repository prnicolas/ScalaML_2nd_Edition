 ...
 */
final class SVMModel(val svmmodel: svm_model, val accuracy: Double) extends Model[SVMModel] {
  ...
  override def toString: String = {
    ...
    if (coefs.nonEmpty) {
      description.append("\nSVM basis functions:\n")
      coefs.foreach(w => {
        w.foreach(c => description.append(s"${format(c, "", SHORT)}\n"))
        description.append("\n")
      })
    }
    // Aggregate the description
    description.append(s"Accuracy: ${format(accuracy, "", SHORT)}\n").toString()
  }
}
