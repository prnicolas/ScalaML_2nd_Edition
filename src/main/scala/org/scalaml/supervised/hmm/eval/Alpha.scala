 ...
 * @since 0.98.2 March 13, 2014
 * @version 0.99.2
 * @see Scala for Machine Learning Chapter 7 Sequential data models / Hidden Markov Model
 * - Evaluation
 */
final private[scalaml] class Alpha protected (lambda: HMMModel, obsSeq: Vector[Int]) extends HMMTreillis(lambda) {
   ...
   * @see Chapter 7 Sequential Data Models / Hidden Markov model / Evaluation / Alpha pass
   */
  def logProb: Option[Double] = alpha.map(m => /:(lambda.numObs, (s, t) => s + Math.log(ct(t)), Math.log(m)))
  ...
  /*
		 ...
		 */
  private def updateAlpha(t: Int): Unit = foreach(lambda.numStates, i =>
