 ...
 */
private[scalaml] case class HMMPrediction(likelihood: Double, states: Array[Int]) {
 ...
 */
final private[scalaml] class HMMModel protected (
    val A: DMatrix,
    val B: DMatrix,
    var pi: Array[Double],
    val numObs: Int
  ...
  def setAlpha(obsSeq: Vector[Int]): DMatrix = {
    ...
    (0 until numStates)./:(DMatrix(numObs, numStates))((m, j) => m += (0, j, pi(j) * B(j, obsSeq.head)))
  }
  ...
  final def getAlphaVal(a: Double, i: Int, obsId: Int): Double = {
    ...
    /:(numStates, (s, n) => s + a * A(i, n)) * B(i, obsId)
  }
  ...
  final def validate(eps: Double): Boolean =
    !((0 until A.nRows).exists(n => Math.abs(A.row(n).sum - 1.0) > eps) ||
      (0 until B.nRows).exists(n => Math.abs(B.row(n).sum - 1.0) > eps) ||
   ...
   */
  final def getBetaVal(b: Double, i: Int, obsId: Int): Double = /:(numStates, (s, k) => s + b * A(i, k) * B(k, obsId))
  ...
  override def toString: String = {
    val piStr = pi.map(x => s"${format(x, "", SHORT)}").mkString(", ")
...
private[scalaml] object HMMModel {
  ...
  def apply(
    ...
    pi: Array[Double],
    numObs: Int
  ): HMMModel = new HMMModel(A, B, pi, numObs)
   ...
   */
  def validate(A: DblMatrix, B: DblMatrix, pi: Array[Double]): Boolean =
    A.length == A.head.length && B.length == A.length && pi.length == A.length
   ...
   * @return true if the dimension of A, B and pi are correct, false otherwise
   */
  def validate(A: DMatrix, B: DMatrix, pi: Array[Double]): Boolean =
  ...
  def validate(model: HMMModel): Boolean =
    model.A.nRows == model.A.nCols && model.B.nCols == model.A.nCols && model.pi.length == model.A.nRows
}
