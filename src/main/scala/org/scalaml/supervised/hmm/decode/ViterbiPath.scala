...
final protected class ViterbiPath(lambda: HMMModel, obsSeq: Vector[Int]) {
   ...
   */
  private[this] val psi = Array.fill(numObs)(Array.fill(numStates)(0))
  private[this] val qStar = new QStar(lambda)
  ...
  val path: HMMPrediction = {
    ...
    def viterbi(t: Int): Double = {
      ...
      (0 until numStates).foreach(updateMaxDelta(t, _))

      // If we reached the last observation... exit by backtracing the
      // computation of the
      if (t == obsSeq.size - 1) {
        val (index, prob) = (0 until numStates).map(i => (i, delta(t, i))).maxBy(_._2)

        // Update the Q* value with the index that maximize the delta.A
        qStar.update(t + 1, index, psi)
        prob
      } else
        viterbi(t + 1)
    }
  ...
  private def updateMaxDelta(t: Int, j: Int): Unit = {
    ...
    // the its state index.
    val (psiValue, index) = (0 until lambda.numStates)
...
object ViterbiPath {
   ...
   * @param obsSeq Array of observations as integer (categorical data)
   */
  def apply(lambda: HMMModel, obsSeq: Vector[Int]): ViterbiPath = new ViterbiPath(lambda, obsSeq)
}
