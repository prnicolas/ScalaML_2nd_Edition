...
final protected[scalaml] class DFTFilter[T: ToDouble](
    fC: Double,
    eps: Double
)(g: F2) extends DFT[T](eps) {
  ...
  private val logger = Logger.getLogger("DFTFilter")
    ...
    case xt: Vector[T] if xt.size >= 2 =>
      ...
      fwrd(xt).map {
        case (trf, freq) =>
          ...
          val filtered: Seq[Double] = freq.indices.map(n => freq(n) * g(n, cutOff))

          // Reconstruct the signal.
          trf.transform(filtered.toArray, INVERSE).toVector
      }
  }
}
 ...
 * @author Patrick Nicolas
 */
protected[scalaml] object DFTFilter {
  type F2 = Function2[Double, Double, Double]

  def apply[T: ToDouble](fC: Double, eps: Double)(g: F2): DFTFilter[T] =
    new DFTFilter[T](fC, eps)(g)

  final val EPS: Double = 1e-3
  def apply[T: ToDouble](
    fC: Double
  )(g: F2): DFTFilter[T] = apply(fC, EPS)(g)
}
