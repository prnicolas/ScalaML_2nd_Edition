 ...
 */
private[scalaml] sealed trait Difference[T] {
  /**
 ...
 * @see Scala for Machine Learning Chap 3, ''Data Pre-processing'' / Time series in Scala
 */
private[scalaml] object Difference {
   ...
   */
  implicit def vector2Double[T]( valuesf: ( DblVec, ( Double, Double ) => T ) ) = new Difference[T] {
    type Result = Vector[T]
    def apply(): Result =
      TSeries.zipWithShift1( valuesf._1 ).collect { case ( next, prev ) => valuesf._2( prev, next ) }
  }
   ...
   * @see org.scalaml.stats.XTSeries.zipWithShift1
   */
  implicit def vector2Series[T]( valuesf: ( VSeries[Double], ( Array[Double], Array[Double] ) => Array[T] ) ) = new Difference[T] {
    type Result = Vector[Array[T]]

    def apply(): Result = TSeries.zipWithShift1( valuesf._1 ).collect { case ( next, prev ) => valuesf._2( prev, next ) }
  }
  ...
  def differentialData[T](
    ...
    target: DblVec,
    f: ( Double, Double ) => T
  ): Try[( VSeries[Double], Vector[T] )] =
    Try( ( TSeries.zipToSeries( x, y, 1 ), difference( ( target, f ) ) ) )
}
