 ...
 *  @version 0.99.2
 */
abstract class TFuturesCallback[T](
    xt: DblVec,
    fct: PfnTransform,
    nPartitions: Int
) extends Controller(xt, fct, nPartitions) with Monitor[Double] {

  protected val logger = Logger.getLogger("TFuturesClbck")
  ...
  private def transform: Array[Future[DblVec]] = {
    ...
    partition.zipWithIndex.foreach {
      case (x, n) =>
        futures(n) = Future[DblVec] { fct(x).get }
    }
    futures
  }
   ...
   */
  protected def reduce(results: Iterable[DblVec]): Seq[Double]
}
