...
final private[scalaml] class Signal(
   ...
   */
  override def toGene(id: String, target: Double, op: Operator): Gene[Double] =
  ...
  override def score: Double =
    if (!operatorFuncMap.contains(op))
      ...
      sumScore(operatorFuncMap.get(op))
  ...
  private def sumScore(f: Option[(Double, Double) => Double]): Double =
  //  f.map(xt.zip(weights).map { case (x, w) => w * _(x, target) }.sum).getOrElse(-1.0)
    f.map( g =>
      (0 until xt.length).map( n =>  weights(n) * g(xt(n), target) ).sum
    ).getOrElse(-1.0)
}
...
private[scalaml] object Signal {
  ...
  def apply(
    ...
    xt: DblVec,
    weights: DblVec
  )(implicit quant: Quantization[Double], encoding: Encoding): Signal =
  ...
  private val MaxTimeSeriesSize = 10000000

  private def check(xt: DblVec, weights: DblVec): Unit = {
    require(xt.nonEmpty, "Signal.check Cannot create a signal with undefined time series input")
    require(
      xt.size < MaxTimeSeriesSize,
