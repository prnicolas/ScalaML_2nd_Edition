...
trait PreprocessingModule[T] {
  ...
  trait Preprocessor[U] {
     ...
     */
    def execute(xt: Vector[U]): Try[DblVec]
  }
   ...
   */
  final class ExpMovingAverage[T: ToDouble](period: Int)(implicit num: Numeric[T])
   ...
   */
  final class DFTFilter[T: ToDouble](
      fc: Double
  )(g: (Double, Double) => Double) extends Preprocessor[T] {
    import scala.language.postfixOps

    private[this] val filter = org.scalaml.filtering.dft.DFTFilter[T](fc, 1e-5)(g)
    private[this] val pfn = filter |>

    override def execute(x: Vector[T]): Try[DblVec] = pfn(x).map(_.toVector)
  }
}
