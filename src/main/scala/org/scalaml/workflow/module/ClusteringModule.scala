...
trait ClusteringModule[T] {
   ...
   */
  final class KMeans[U: ToDouble](
      K: Int,
      maxIters: Int,
      distance: (Array[Double], Array[U]) => Double,
      xt: VSeries[U]
    ...
    private[this] val kmeans = unsupervised.kmeans.KMeans[U](K, maxIters, distance, xt)
   ...
   */
  final class MultivariateEM[T: ToDouble](K: Int, xt: VSeries[T]) extends Clustering[T] {
