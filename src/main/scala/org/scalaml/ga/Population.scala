 ...
 */
private[scalaml] case class GeneticIndices(chOpIdx: Int, geneOpIdx: Int) {
 ...
 * components
 */
private[scalaml] class Population[U: ToDouble, T <: Gene[U]](limit: Int, val chromosomes: Pool[U, T]) {
  import Population._
  ...
  def +(that: Population[U, T]): Population[U, T] = {
    ...
    if (that.size > 0) Population[U, T](limit, chromosomes ++: that.chromosomes) else this
  }
  ...
  protected def +=(newCode: List[T]): Unit = {
    ...
    require(
      newCode.size == chromosomes.head.size,
  ...
  def select(score: Chromosome[U, T] => Unit, cutOff: Double): Unit = {
    ...
    val cumul = chromosomes./:(0.0)((s, c) => s + c.cost) / ScalingFactor
  ...
  //  val cumul = chromosomes.map(_.cost).sum / ScalingFactor
    ...
    val newChromosomes = chromosomes.sortBy(_.cost)
   ...
   */
  @throws(classOf[IllegalArgumentException])
  final def diff(that: Population[U, T], depth: Int): Option[Pool[U, T]] = {
   ...
   */
  @throws(classOf[IllegalArgumentException])
  final def fittest(depth: Int): Option[Pool[U, T]] = {
    require(
      depth > 0,
   ...
   */
  final def fittest: Option[Chromosome[U, T]] = if (size > 0) Some(chromosomes.head) else None
 ...
 */
private[scalaml] object Population {
   ...
   * @param chromosomes Current pool of chromosomes (type: ArrayBuffer{Chromosome[T])
   */
  def apply[U: ToDouble, T <: Gene[U]](limit: Int, chromosomes: Pool[U, T]): Population[U, T] =
    new Population[U, T](limit, chromosomes)
   ...
   */
  def apply[U: ToDouble, T <: Gene[U]](limit: Int, chromosomes: List[Chromosome[U, T]]): Population[U, T] =
    new Population[U, T](limit, mutable.ArrayBuffer[Chromosome[U, T]]() ++ chromosomes)

  private val MAX_NUM_CHROMOSOMES = 10000

  private def check[U: ToDouble, T <: Gene[U]](limit: Int, chromosomes: Pool[U, T]): Unit = {
    require(
      chromosomes.nonEmpty,
   ...
   */
  def nullPopulation[U: ToDouble, T <: Gene[U]]: Population[U, T] = new Population[U, T](-1, ArrayBuffer.empty)
}
