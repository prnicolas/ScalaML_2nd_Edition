...
private[scalaml] final class GAConfig(
    val xover: Double,
    val mu: Double,
    val maxCycles: Int,
 ...
 * configuration
 */
private[scalaml] object GAConfig {
  ...
  private val DefaulSoltLimit = (n: Int) => -0.01 * n + 1.001
  ...
  def apply(xover: Double, mu: Double, maxCycles: Int): GAConfig =
    new GAConfig(xover, mu, maxCycles, DefaulSoltLimit)
