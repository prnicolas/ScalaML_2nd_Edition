 ...
 * @since 0.97 September 12, 2013
 * @version 0.99.2
 * @see Scala for Machine Learning Chapter 13 Evolutionary Computing / Genetic Algorithm Components
 */
private[scalaml] trait Operator {
  /**
...
private[scalaml] class Gene[U: ToDouble](
    val id: String,
    val target: U,
    val op: Operator
)(implicit quant: Quantization[U], encoding: Encoding) {
  ...
  def apply(value: U, operator: Operator): util.BitSet = {
  ...
  def unapply(bitSet: util.BitSet): (U, Operator) =
    (quant.toU(convert(encoding.rValue, bits)), op(convert(encoding.rOp, bits)))

  /**
   * Create a clone of this gene by duplicating its genetic material (bits).
   * @return identical Gene
   */
  override def clone: Gene[U] = (0 until bits.length)./:(Gene(id, target, op))((enc, n) => {
      if (bits.get(n))
        enc.bits.set(n)
      enc
    })
   ...
   */
  def ^(indices: GeneticIndices): Gene[U] = ^(indices.geneOpIdx)
...
private[scalaml] object Gene {
   ...
   */
  def apply[U: ToDouble](
    id: String,
    target: U,
    op: Operator)(implicit quant: Quantization[U], encoding: Encoding): Gene[U] = new Gene[U](id, target, op)
