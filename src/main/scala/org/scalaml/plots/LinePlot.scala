 ...
 */
final private[scalaml] class LinePlot(legend: Legend, theme: PlotTheme) extends Plot(legend, theme) {
  ...
  def display(
    xys: immutable.List[(DblVec, String)],
    w: Int,
    h: Int
  ): Boolean = {
    val validDisplay = validateDisplay[List[(DblVec, String)]](xys, w, h, "LinePlot.display")
    if (validDisplay) {
 ...
 * @see http://www.jfree.org
 */
private[scalaml] object LinePlot {
  import BasicStroke._, DisplayUtils._
  ...
  def display(
    ...
    createPlot(isChart, legend, theme).display(y.toArray, DEFAULT_WIDTH, DEFAULT_WIDTH)
  ...
  def display(
    ...
    createPlot(isChart, legend, theme).display(y, DEFAULT_WIDTH, DEFAULT_WIDTH)
  ...
  def display(
    ...
    createPlot(isChart, legend, theme).display(xys, DEFAULT_WIDTH, DEFAULT_WIDTH)
