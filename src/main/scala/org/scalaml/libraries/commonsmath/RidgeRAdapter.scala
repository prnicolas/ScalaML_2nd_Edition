...
private[scalaml] class RidgeRAdapter(lambda: Double, dim: Int) extends AbstractMultipleLinearRegression {

  // The linear optimization method (QR, Cholesky...) is selected at run time.
  private[this] var qr: QRDecomposition = _

  def createModel(x: DblMatrix, y: DblVec): Unit = {
    this.newXSampleData(x)
    super.newYSampleData(y.toArray)
  }

  final def getWeights: Array[Double] = calculateBeta.toArray
  ...
  override protected def newXSampleData(x: DblMatrix): Unit = {
    ...
    Range(0, dim).foreach(i => xtx.setEntry(i, i, xtx.getEntry(i, i) + lambda))
