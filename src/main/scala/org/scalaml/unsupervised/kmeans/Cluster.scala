...
private[scalaml] final class Cluster[T: ToDouble](val center: Array[Double]) {
  ...
  final def moveCenter(
    ...
    Cluster[T](sums.map(implicitly[ToDouble[T]].apply(_) / members.size))
  }
  ...
  @throws(classOf[IllegalArgumentException])
  @throws(classOf[IllegalStateException])
  final def stdDev(xt: Vector[Array[T]], distance: DistanceFunc[T]): Double = {
    require(
      xt.nonEmpty,
    ...
    val ts: Vector[Double] = members.map(xt(_)) // convert a vector
  ...
  private def show(x: Double): String = format(x, "", SHORT)
}
 ...
 */
private[scalaml] object Cluster {
  type DistanceFunc[T] = (Array[Double], Array[T]) => Double

  final def euclidDistance[T: ToDouble](x: Array[Double], y: Array[T]) = euclidean(x, y)

  /**
   * Default constructor for a cluster
   * @param center Initial centroid for this cluster
   */
  def apply[T: ToDouble](center: Array[Double]): Cluster[T] = new Cluster[T](center)

  /**
   * Constructor for a Cluster with undefined center.
   */
  def apply[T: ToDouble]: Cluster[T] = new Cluster[T](Array.empty[Double])
}
