...
private[scalaml] object FormatUtils {
  ...
  def format(x: Array[Double]): String = format(x.toArray)
  ...
  def format(m: DblMatrix, fmt: FormatType): String = {
    ...
    val content = m.indices.map(i => s"${fmt.toString(i)} ${
      m.head.indices.map(j => s"${fmt.toString(m(i)(j))}").mkString
    }").mkString("\n")
    s"$header\n$content"
  }
   ...
   */
  def toText(v: Array[Double], index: Boolean): String = {
    require(
      !v.isEmpty,
