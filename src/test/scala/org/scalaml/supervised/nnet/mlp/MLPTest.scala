...
final class MLPTest extends FlatSpec with Matchers with Resource with Logging {
  ...
  it should s"$name bimodal classification with two hidden layer" in {
    ...
    evaluate(Array[Int](4, 4))
  }

  it should s"$name Binary classification two hiddenlayers impact of alpha and eta factors" in {
    import scala.language.postfixOps
    show(s"$name Binary classification two hiddenlayers impact of alpha and eta factors")
    ...
    LinePlot.display(data2, legend2, new LightPlotTheme)
  }

  private def evaluate(hiddenLayers: Array[Int]): Unit = {
    val dataPath = "nnet/mlp"
    val ALPHA = 0.8
    val ETA = 0.05
    val NUM_EPOCHS = 2500
    val EPS = 1e-6
    ...
    val prices = symbols.map(s => DataSource( s"${getPath(s"$dataPath/$s.csv")}", true, true, 1))
